...
def obsflux_Ia(filter, z_Ia, my_phase):
...
def obsflux_Ibc(filter, z_cc, my_phase): 
    """Given a filter, redshift z at given phase, generates the observed 
...
def obsflux_II(filter, z_cc, my_phase): 
...
def mc_file(n, filter, z_Ia, z_cc):
    ...
    for i in range(n):
        ...
        my_obsflux_Ia = obsflux_Ia(filter, z_Ia, my_phase)
        type_Ia_flux[i] = my_obsflux_Ia

        my_obsflux_Ibc= obsflux_Ibc(filter, z_cc, my_phase)
        type_Ibc_flux[i] = my_obsflux_Ibc

        my_obsflux_II = obsflux_II(filter, z_cc, my_phase)
        type_II_flux[i] = my_obsflux_II


    fname = '/Users/carolinesofiatti/projects/scp/flux_vs_fluxdiff/files/z%0.2f' % z_Ia + '_'+ filter +'_mc.gz'
